...
async function withLsp<T>(
  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, rej) => setTimeout(() => rej(new Error("initialize timeout")), timeoutMs)),
    ]);
...
export default function lspExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      else if (contents && typeof contents === "object" && "value" in contents) text = contents.value;
      return ok(text);
    },
  });
