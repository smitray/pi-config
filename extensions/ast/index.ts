...
export default function astExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    label: "AST Search",
    description: "Search code by AST pattern (semantic). Prefer over text grep for structured patterns.",
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const args = ["run", "--pattern", params.pattern, "--rewrite", params.rewrite, "--json=compact"];
      if (params.lang) args.push("--lang", params.lang);
      if (params.paths?.length) args.push(...params.paths);
